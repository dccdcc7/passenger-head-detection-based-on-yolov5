...
class SEConv(nn.Module):
    ...
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, act=True,
                 reduction=16):  # ch_in, ch_out, kernel, stride, padding, groups
        super().__init__()
...
class C2f_SE(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5):  # ch_in, ch_out, number, shortcut, groups, expansion
        ...
        self.m = nn.ModuleList(SEBottleneck(self.c, self.c, shortcut, g) for _ in range(n))
...
class C3SE(C3):
    ...
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):
        ...
        c_ = int(c2 * e)  # hidden channels
        self.m = nn.Sequential(*(BottleneckSE(c_, c_, shortcut) for _ in range(n)))
...
class Bottleneck1_ScConv(Bottleneck):
    def __init__(self, c1, c2, shortcut=True, g=1, e=0.5):
        super().__init__(c1, c2, shortcut, g)
        c_ = int(c2 * e)  # hidden channels
        self.cv1 = Conv(c1, c_, 3, 1)
        self.cv2 = ScConv(c2)

class C3_S(nn.Module):
    # CSP Bottleneck with 3 convolutions
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):
        """Initializes C3 module with options for channel count, bottleneck repetition, shortcut usage, group
        convolutions, and expansion.
        ...
        self.cv2 = Conv(c1, c_, 1, 1)
        self.cv3 = Conv(2 * c_, c2, 1)  # optional act=FReLU(c2)
        self.m = nn.Sequential(*(Bottleneck1_ScConv(c_, c_, shortcut, g, e=1.0) for _ in range(n)))
...
class Bottleneck_ScConv(Bottleneck_v8):
...
class C2f(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5):  # ch_in, ch_out, number, shortcut, groups, expansion
        ...
        self.cv1 = Conv(c1, 2 * self.c, 1, 1)
        self.cv2 = Conv((2 + n) * self.c, c2, 1)  # optional act=FReLU(c2)
        self.m = nn.ModuleList(Bottleneck_v8(self.c, self.c, shortcut, g, k=((3, 3), (3, 3)), e=1.0) for _ in range(n))
...
if __name__=="__main__":
   input = random_tensor = torch.rand(16, 16, 640, 640)
   # c2f= C2f(c1=16, c2=64, n=1)
   # c2f1 = C2f_S(c1=16, c2=64, n=1)
   layer = C3_S(c1=16, c2=64, n=1)
   #bn = SEBottleneck(c1=16, c2=64)
